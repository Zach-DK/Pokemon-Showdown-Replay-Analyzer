...
def normalize_species(s):
    """
    Remove trailing gender markers (e.g., ", M" or ", F") so that
    species are printed without them.
    """
    s = s.strip()
...
def main():
    while True:
        ...
        # The highest turn number is the battle length.
        battle_turns = turns[-1][0] if turns else 0

        # Tracking for kills and sources.
        kills = []  # Each entry: (inflictor_key, fainted_key, turn, cause_of_death)
        ...
        for attacker_key, victim_key, turn, cause in kills:
            kill_type = "passive" if cause in ({"Stealth Rock", "Spikes", "Toxic Spikes", "Sticky Web", "brn", "psn", "Salt Cure", "Life Orb"} | partial_trap_moves) else "direct"
            if attacker_key:
        ...
        # Now, aggregate stats by team and normalized species.
        species_stats = {}
        ...
        def print_table(title, species_dict):
            print(f"\n=== {title} ===")
            header = f"{'Species':<25} {'Direct Kills':<15} {'Passive Kills':<15} {'Direct Deaths':<15} {'Passive Deaths':<15}"
            print(header)
            print("-" * len(header))
            for rec in species_dict:
                print(f"{rec['species']:<25} {rec['direct_kills']:<15} {rec['passive_kills']:<15} {rec['direct_deaths']:<15} {rec['passive_deaths']:<15}")

        print_table(f"{p1_name}'s stats", p1_species_stats)
        print_table(f"{p2_name}'s stats", p2_species_stats)
        # Print additional battle info (turn count and winner)
